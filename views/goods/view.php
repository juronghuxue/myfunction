...
<meta property="og:site_name" content="Elegomall.com" >
...
<div class="container" >
                    ...
                    <div class="inline" style="width:100%;">    
                        <div class="detail_img" id="detail_img" style="width:100%;">
                            <div class="pic_view" style="">
                                <img itemprop="image" id="pic-view" alt="<?= $model->name ?>" src="<?= $img_host.$model->image ?>" onerror="javascript:this.src='/theme/new/images/noimg_quesheng.png';" id="bigImg" jqimg="<?= $img_host.$model->image ?>" style="border:none;">
                                <!-- <img src="/theme/new/images/pic_merry_christmas.png" style="position:absolute;top:10px;left:10px;"/> -->
                                <p class="hover-zoom">hover to zoom</p>
                                <div id="zoom-zhezhao" class="zoom-zhezhao" style="display:none;"></div>
                            </div>
                            <div class="clearfix m_pic">
                                <ul class="clearfix">
                                    <?php
                                ...
                                <!-- <img src="/theme/new/images/pic_merry_christmas.png" style="position:absolute;top:0;left:10px;"/> -->
                                <img class="m_begain" src="/images/product_view/icon_jiantou_left_product_image.png" style="position:absolute;left:0;top:0px;">
                                <img class="m_end" src="/images/product_view/icon_jiantou_right_poduct_imge.png" style="position:absolute;right:0;top:0px;">
                            </div>                    
                    ...
                    <div class="goods_detail"><!-- wd50p divr -->
                        ...
                        <div class="price_info">
                            <p itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="price" style="margin-bottom:0;position:relative;background:#fef9f5;border:1px dashed #e3e5e7;border-bottom:none;">
                                ...
                                <span class="price_left">Price:&nbsp;<i class="new_price" itemprop="price"><?php if($model->price>0):?>$<?= $model->price?><?php else:?>Coming soon<?php endif;?></i>&nbsp;
                            <?php if(1){?>
                                <i class="old_price" style="text-decoration:line-through"><?php if($old_price>0):?>$<?= $old_price?><?php endif;?></i>
                            <?php }?>
                                </span>
                                <span style="font-size:14px;" class="detail_mobile_app">
                                    <img src="/theme/new/images/icon_mobile.png"/>&nbsp;&nbsp;&nbsp;Buy on Mobile App
                                </span>
                                <strong class="sale-off">&nbsp;&nbsp;<?php if($old_price>0 and $model->price>0):?><?=(($old_price-$model->price)/$old_price*100)?><?php endif;?>% OFF</strong>
                            </p>       
                            <?php if(!$user){?>
                            ...
                            <div class="detail_sao_wrap">
                                <div class="detail_sao">
                                    <img src="/theme/new/images/ios-adrs.png" />
                        ...
                        <div class="goods_number clearfix">
                            ...
                            <div class="avail">
                                <span class="avail_name" data-preOrderTime="<?= $model->pre_order_time > 0 ? date("Y-m-d", $model->pre_order_time) : '' ?>">Availbility:</span>
    ...
    <?php //品牌不能出售限制国家
    $limitBrandCountry = \common\models\Cart::getLimitBrandCountry();
    if (!empty($limitBrandCountry) && is_array($limitBrandCountry) && isset($limitBrandCountry[$model->brand_id])) {
        $country = \common\models\Region::find()->where(['in', 'code', $limitBrandCountry[$model->brand_id]])->select("name")->asArray()->all();
        $countryNames = array_column($country, "name");
        $countryName = implode("、", $countryNames);
        ?>
        <div id="explain" class="clearfix">
            <div class="left-ico"><img src="/theme/new/images/quan.png" /></div>
            <div class="right-desc">
                <span>Attention:</span>
                <span>Shipping restriction: Due to exclusive distributor of <?=$model->brand->name ?>, we do not send <?=$model->brand->name ?> products to customers in <?=$countryName?>. <!--You can check more <a href="javascript:;">exclusive distributors</a>.--></span>
            </div>
        </div>
    <?php }?>
        ...
        <div class="clearfix tabs_bar_warp" style="background:#ffffff;">
            <div  id="tabs_bar" class="clearfix ">
                ...
                <script>
                    ...
                    (function(){
                        var m_pic = $('.m_pic');
                        var m_pic_width = m_pic.width();
                        var m_pic_height = m_pic.height();
                        var ul = $('.m_pic ul');
                        var num = 0;
                        m_pic.css('height',m_pic_width);
                        ul.find('li').css('width',m_pic_width);
                        var li_length = ul.find('li').length;
                        var ul_length = ul.find('li').length*m_pic_width;
                        $('.m_pic ul').css('width',ul_length);
                        $('.m_begain').css('top',m_pic_width/2+12+'px');
                        $('.m_end').css('top',m_pic_width/2+12+'px');

                        $('.m_begain').on('click',function(){
                            num--;
                            if(num<0){
                                num = 0;
                                layer.msg('This is the first picture',{icon:0,time:1000});
                                return;
                            }
                            ul.stop().animate({'left':(-num)*m_pic_width+'px'},200);
                        });

                        $('.m_end').on('click',function(){
                            num++;
                            if(num>=li_length){
                                num = li_length-1;
                                layer.msg('This is the last picture',{icon:0,time:1000});
                                return;
                            }
                            ul.stop().animate({'left':(-1)*m_pic_width*num+'px'},200);
                        });
                    })();
...
// <div>Christmas sale</div>
    ...
    var type = <?= $type?>;//消费者权益
    console.log(type);
    var div_small_sure = '<div class="original">100% Original</div>';
    for(var i = 0;i<type.length;i++){    
        if(type[i] == 'IS_TPD'){
            div_small_sure += '<div class="compliance">TPD Compliance</div>';
        }else if(type[i] == 'IS_TPD_PAKGE'){
            div_small_sure += '<div class="package">TPD Package</div>';
        }
    }
    ...
    $('.opt_select').eq(0).change(function(){
        ...
        is_piliang(HeightLightCart,id,option1);
            ...
            for(var i = 0;i<$('.child_ids').length;i++){
                if(group_id == $('.child_ids').eq(i).val()){
                    ...
                    if(num_old_price === num_new_price){
                        ...
                        if(discount_off_new == 0 || isNaN(num_new_price)){
    ...
    //页面购物车产品信息
    function HeightLightCart(id,opton1){
        ...
        if(id !='' && !opton1){
            for(var i = 0;i<child_ids.length;i++){
                if(child_ids.eq(i).val()==id && child_ids.eq(i).closest('.quantity').siblings('.stock_status').text() =='In stock'){
                    ...
                    preorder(i,child_ids.eq(i).closest('tr').attr('data-preordertime'));//pre-order时间
...
// console.log(ispreorder,isparent);
    function initHeightLightCartDan(){
        ...
        if(isSubProduct == 0 && isPreOrderTime > 0){ // 是单品是pre-order
            ...
            preorder(0,$('.avail_name').attr('data-preordertime'));
    ...
    function instock(x){
        $('.avail_static').empty().text('In stock');//child_ids.eq(i).closest('.quantity').siblings('.stock_status').text()
        if(UNIT_TYPE !='pc'){
    ...
    function preorder(x,str){//str:pre-order时间的修改
        if(str){
            $('.avail_static').empty().text(year_month_date(str));
        }else{
            $('.avail_static').empty().text('Pre-order');
    ...
    function year_month_date(str){
         var year_month_datem = str.split('-');
         var iMonth = '';
         var iDate = '';
         switch(year_month_datem[1])
         {
             case '01':
                 iMonth = 'January';
                 break;
         ...
         if(Number(year_month_datem[2])<10){
             iDate = year_month_datem[2].substring(1);
         }else{
             iDate = year_month_datem[2];
         }
         return iMonth+' '+iDate+'(ETA)';
        }
    var UNIT_TYPE = isAPP();
    if(UNIT_TYPE != 'pc'){
        ...
        $('#spxqitem iframe').css({'width':'100%','height':'auto'});
    }
...
//图片100%
    $('#spxqitem img').css({'width':'100%','height':'auto'});
</script>
